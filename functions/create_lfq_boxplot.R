...
# Define function
create_lfq_boxplot <- function(se, col_data, group_col = "frailty",
                                     sample_col = "sample", 
  ...
  rownames(se_df) <- NULL
  se_long <- melt(se_df)
  colnames(se_long) <- c("protein_id", "Samples", lfq_type)
  ...
  plot_1 <- ggplot(se_long, aes(x = Samples,
                                y = .data[[lfq_type]],
                                fill = .data[[group_col]])) + 
  ...
  # Return
  return(list(se_long = se_long, lfq_summary = lfq_summary,
