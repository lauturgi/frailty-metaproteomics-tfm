...
# ==================================
# Limma analysis
# Preprocessing: Norm MaxLFQ, log2transformed, NA per condition < 0.5, no imputation
# ==================================

# Load filtered SummarizedExperiment
work_path <- getwd()
load(paste0(work_path, "/data/se_norm.RData"))
...
diff_1 <- topTable(fit1, coef=2, adjust="BH", sort.by="P", number=nrow(assay(se_norm)))
...
nrow(diff_1) 
rownames(diff_1)
# 8 proteins (p.adj < 0.05): P06702, A6KYJ0, A6L792, P05109, Q8A9M2, P94360, C4KZP0, A9KJL3
...
sel0 = which("P94360"==rownames(assay(se_norm)))
df0 = data.frame(frailty=colData(se_norm)[,c("frailty")], expression=assay(se_norm)[sel0,])
...
design = model.matrix(~ colData(se_norm)$frailty + colData(se_norm)$sex)
...
diff_2 <- topTable(fit1,coef=2, adjust ="BH", sort.by="P", number=nrow(assay(se_norm)))
diff_3 <- topTable(fit1,coef=3, adjust ="BH", sort.by="P", number=nrow(assay(se_norm)))
...
nrow(diff_2) 
rownames(diff_2)
# 5: P06702, A6KYJ0, A6L792, P05109, C4KZP0
...
diff_4 <- topTable(fit1,coef=2, adjust ="BH", sort.by="P", number=nrow(assay(se_norm)))
diff_5 <- topTable(fit1,coef=3, adjust ="BH", sort.by="P", number=nrow(assay(se_norm)))
diff_6 <- topTable(fit1,coef=4, adjust ="BH", sort.by="P", number=nrow(assay(se_norm)))
...
nrow(diff_4) 
rownames(diff_4)
# 23: A6KYJ0, C4KZP0, A6L792, Q5L923, A6KYJ7, P05109, P95544, Q5L9B6, C4ZBD3, C4ZD46, Q8A9M2, Q8A1A2, A6KYH8, A9KKU0, P55990, C4ZI85, C4ZBG1, P06702, A6KYH1, A6KYJ6, C4ZF71, Q18CF4, C4Z0Q6
...
sel0 = which("C4Z0Q6"==rownames(assay(se_norm)))
df0 = data.frame(frailty=colData(se_norm)[,c("frailty")], 
                 sex=colData(se_norm)[,c("sex")], 
                 expression=assay(se_norm)[sel0,])
...
se_norm_alcohol <- se_norm[, !is.na(colData(se_norm)$alcohol)]
dim(se_norm_alcohol)
design = model.matrix(~ colData(se_norm_alcohol)$frailty + colData(se_norm_alcohol)$alcohol)
colnames(design) = c("constant", "frailty", "alcohol_monthly", "alcohol_weekly")
head(design)

# Fit a linear model for each row of the expression matrix
fit = lmFit(assay(se_norm_alcohol), design)
fit1 = eBayes(fit)
head(coef(fit1))

# 4 coefficients per each column of the model matrix. Adjusted using the Benjamini-Hochberg
diff_7 <- topTable(fit1,coef=2, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_alcohol)))
diff_8 <- topTable(fit1,coef=3, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_alcohol)))
diff_9 <- topTable(fit1,coef=4, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_alcohol)))
...
diff_10 <- topTable(fit1,coef=2, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_alcohol)))
diff_11 <- topTable(fit1,coef=3, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_alcohol)))
diff_12 <- topTable(fit1,coef=4, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_alcohol)))
diff_13 <- topTable(fit1,coef=5, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_alcohol)))
diff_14 <- topTable(fit1,coef=6, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_alcohol)))
...
sel0 = which("P22983"==rownames(assay(se_norm_alcohol)))
df0 = data.frame(frailty=colData(se_norm_alcohol)[,c("frailty")], 
                 alcohol=colData(se_norm_alcohol)[,c("alcohol")], 
                 expression=assay(se_norm_alcohol)[sel0,])
...
se_norm_tobacco <- se_norm[, !is.na(colData(se_norm)$tobacco)]
dim(se_norm_tobacco)
design = model.matrix(~ colData(se_norm_tobacco)$frailty + colData(se_norm_tobacco)$tobacco)
colnames(design) = c("constant", "frailty", "former", "current")
head(design)

# Fit a linear model for each row of the expression matrix
fit = lmFit(assay(se_norm_tobacco), design)
fit1 = eBayes(fit)
head(coef(fit1))

# 4 coefficients per each column of the model matrix. Adjusted using the Benjamini-Hochberg
diff_15 <- topTable(fit1,coef=2, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_tobacco)))
diff_16 <- topTable(fit1,coef=3, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_tobacco)))
diff_17 <- topTable(fit1,coef=4, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_tobacco)))
...
nrow(diff_15) 
rownames(diff_15)
# 6: P06702, A6KYJ0, P94360, A6L792, P05109, Q8A9M2
...
# 6 coefficients per each column of the model matrix. Adjusted using the Benjamini-Hochberg
diff_18 <- topTable(fit1,coef=2, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_tobacco)))
diff_19 <- topTable(fit1,coef=3, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_tobacco)))
diff_20 <- topTable(fit1,coef=4, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_tobacco)))
diff_21 <- topTable(fit1,coef=5, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_tobacco)))
diff_22 <- topTable(fit1,coef=6, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_tobacco)))
...
nrow(diff_22) 
rownames(diff_22)
# 79: A6KYH6, P0C2E7, C4ZBT7, C4ZBL1, A6KYK2, C4ZB90, A6L1X1, A6KYJ7, A9VT65, A9KJL3, A9KRZ2, A6LFQ4, P22983, A6KYH8, C4Z2R9, C4ZBU3, C4ZD46, C4ZBS5, C4ZBD3, C4Z2T1, C4Z3R4, A9KK92, Q59309, A6KYK6, A9KK94, A6KYI1, A6TH53, C4ZBG1, Q8A9M2, P0C0G6, A6KXA0, C4ZBT1, A6KYH0, P94316, A6KYJ8, Q8RQP4, A9KJI8, C4Z2T8, C4ZBS1, A9KKU0, A6KYJ2, Q88XX2, A6L048, Q8A1A2, A6L1L8, C0R090, A9KRZ3, C4ZBD5, A6KXL2, A6KYK3, B8I7Y6, C4Z1J4, A6L792, P95544, C4KZP0, A6TWI7, A9KJI0, A6KYI8, P19543, A6KYK9, A6KYJ6, Q5L9B6, A6L0V1, A6L903, P02768, Q5L923, C4ZBS3, Q18CF4, A4XI37, C4ZF71, A5I7J4, P24295, C4Z2S9, A4W5A0, A6L5A6, A9KQ65, P94598, P55990, C4Z2T9

sel0 = which("C4ZBT7"==rownames(assay(se_norm_tobacco)))
df0 = data.frame(frailty=colData(se_norm_tobacco)[,c("frailty")], 
                 tobacco=colData(se_norm_tobacco)[,c("tobacco")], 
                 expression=assay(se_norm_tobacco)[sel0,])
...
to_keep <- rowSums(!is.na(colData(se_norm)[, sig_var])) == length(sig_var)
se_norm_all <- se_norm[, to_keep]
dim(se_norm_all)

# Build model matrix considering all predictor variables and interaction
design = model.matrix(~ colData(se_norm_all)$frailty*colData(se_norm_all)$sex +
                        colData(se_norm_all)$frailty*colData(se_norm_all)$alcohol + 
                        colData(se_norm_all)$frailty*colData(se_norm_all)$tobacco +
                        colData(se_norm_all)$frailty*colData(se_norm_all)$diabetes +
                        colData(se_norm_all)$frailty*colData(se_norm_all)$chf +
                        colData(se_norm_all)$frailty*colData(se_norm_all)$depression +
                        colData(se_norm_all)$frailty*colData(se_norm_all)$osteoarthritis +
                        colData(se_norm_all)$frailty*colData(se_norm_all)$sarcopenia +
                        colData(se_norm_all)$frailty*colData(se_norm_all)$ilef +
                        colData(se_norm_all)$frailty*colData(se_norm_all)$bmi +
                        colData(se_norm_all)$frailty*colData(se_norm_all)$energy)
...
# Fit a linear model for each row of the expression matrix
fit = lmFit(assay(se_norm_all), design)
fit1 = eBayes(fit)
head(coef(fit1))

# 6 coefficients per each column of the model matrix. Adjusted using the Benjamini-Hochberg
diff_23 <- topTable(fit1,coef=2, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_all)))
diff_24 <- topTable(fit1,coef=3, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_all)))
diff_25 <- topTable(fit1,coef=4, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_all)))
diff_26 <- topTable(fit1,coef=5, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_all)))
diff_27 <- topTable(fit1,coef=6, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_all)))
diff_28 <- topTable(fit1,coef=7, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_all)))
diff_29 <- topTable(fit1,coef=8, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_all)))
diff_30 <- topTable(fit1,coef=9, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_all)))
diff_31 <- topTable(fit1,coef=10, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_all)))
diff_32 <- topTable(fit1,coef=11, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_all)))
diff_33 <- topTable(fit1,coef=12, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_all)))
diff_34 <- topTable(fit1,coef=13, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_all)))
diff_35 <- topTable(fit1,coef=14, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_all)))
diff_36 <- topTable(fit1,coef=15, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_all)))
diff_37 <- topTable(fit1,coef=16, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_all)))
diff_38 <- topTable(fit1,coef=17, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_all)))
diff_39 <- topTable(fit1,coef=18, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_all)))
diff_40 <- topTable(fit1,coef=19, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_all)))
diff_41 <- topTable(fit1,coef=20, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_all)))
diff_42 <- topTable(fit1,coef=21, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_all)))
diff_43 <- topTable(fit1,coef=22, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_all)))
diff_44 <- topTable(fit1,coef=23, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_all)))
diff_45 <- topTable(fit1,coef=24, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_all)))
diff_46 <- topTable(fit1,coef=25, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_all)))
diff_47 <- topTable(fit1,coef=26, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_all)))
diff_48 <- topTable(fit1,coef=27, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_all)))
diff_49 <- topTable(fit1,coef=28, adjust ="BH", sort.by="P", number=nrow(assay(se_norm_all)))
...
diff_26_up <- diff_26[diff_26$logFC > 0,]
# 1
diff_26_down <- diff_26[diff_26$logFC < 0,]
# 0
...
nrow(diff_37) 
rownames(diff_37)
# 6: P95544, A6KYJ0, Q189R2, A6KYH1, A6KYJ7, Q59309

sel0 = which("Q189R2"==rownames(assay(se_norm_all)))
df0 = data.frame(frailty=colData(se_norm_all)[,c("frailty")], 
                 sex=colData(se_norm_all)[,c("sex")], 
                 expression=assay(se_norm_all)[sel0,])
df0$frailty_sex <- paste(df0$frailty, df0$sex, sep = "_")
ggplot(df0,aes(x=frailty_sex,y=expression)) + geom_boxplot()

diff_37_up <- diff_37[diff_37$logFC > 0,]
# 0
diff_37_down <- diff_37[diff_37$logFC < 0,]
# 6
...
sel0 = which("A6KXA0"==rownames(assay(se_norm_all)))
df0 = data.frame(frailty=colData(se_norm_all)[,c("frailty")], 
                 tobacco=colData(se_norm_all)[,c("tobacco")], 
                 expression=assay(se_norm_all)[sel0,])
df0$frailty_tobacco <- paste(df0$frailty, df0$tobacco, sep = "_")
ggplot(df0,aes(x=frailty_tobacco,y=expression)) + geom_boxplot()

diff_41_up <- diff_41[diff_41$logFC > 0,]
# 1
diff_41_down <- diff_41[diff_41$logFC < 0,]
# 0
...
sel0 = which("Q59309"==rownames(assay(se_norm_all)))
df0 = data.frame(frailty=colData(se_norm_all)[,c("frailty")], 
                 bmi=colData(se_norm_all)[,c("bmi")], 
                 expression=assay(se_norm_all)[sel0,])
